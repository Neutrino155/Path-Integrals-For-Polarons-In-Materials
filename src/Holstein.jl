...
function holstein(α::Number, ω::Number, J::Number; dims = 3, verbose = false)
    ...
    S(v, w) = holstein_S(v, w, g, τ -> phonon_propagator(τ, ω), (τ, v, w) -> J * polaron_propagator(τ, v, w); dims = dims)
...
function holstein(α, ω::Number, J::Number; dims = 3, verbose = false)
    ...
    for i in 1:num_α
        ...
        S(v, w) = holstein_S(v, w, g[i], τ -> phonon_propagator(τ, ω), (τ, v, w) -> J * polaron_propagator(τ, v, w); dims = dims)
...
function holstein(α::Number, ω, J::Number; dims = 3, verbose = false)
    ...
    for i in 1:num_ω
        ...
        S(v, w) = holstein_S(v, w, g[i], τ -> phonon_propagator(τ, ω[i]), (τ, v, w) -> J * polaron_propagator(τ, v, w); dims = dims)
...
function holstein(α, ω, J::Number; dims = 3, verbose = false)
    ...
    for i in 1:num_α, j in 1:num_ω
        ...
        S(v, w) = holstein_S(v, w, g[i, j], τ -> phonon_propagator(τ, ω[j]), (τ, v, w) -> J * polaron_propagator(τ, v, w), dims = dims)
...
function holstein(α::Number, ω::Number, J::Number, β::Number; dims = 3, verbose = false)
    ...
    g = pustrip(holstein_coupling(α, ω * ω0_pu, J * E0_pu, dims))
    S(v, w) = holstein_S(v, w, g, τ -> β == Inf ? phonon_propagator(τ, ω) : phonon_propagator(τ, ω, β), (τ, v, w) -> β == Inf ? J * polaron_propagator(τ, v, w) : J * polaron_propagator(τ, v, w, β); limits = [0, β / 2], dims = dims)
    ...
    Σ = holstein_memory(eps(), g, t -> β == Inf ? phonon_propagator(t, ω) : phonon_propagator(t, ω, β), t -> β == Inf ? J * polaron_propagator(t, v, w) : J * polaron_propagator(t, v, w, β), dims = dims) * ω
...
function holstein(α, ω::Number, J::Number, β::Number; dims = 3, verbose = false)
    ...
    @inbounds for i in 1:num_α
        ...
        S(v, w) = holstein_S(v, w, g[i], τ -> β == Inf ? phonon_propagator(τ, ω) : phonon_propagator(τ, ω, β), (τ, v, w) -> β == Inf ? J * polaron_propagator(τ, v, w) : J * polaron_propagator(τ, v, w, β), limits = [0, β/2]; dims = dims)
        ...
        @fastmath Σ[i] = holstein_memory(eps(), g[i], t -> β == Inf ? phonon_propagator(t, ω) : phonon_propagator(t, ω, β), t -> β == Inf ? J * polaron_propagator(t, v[i], w[i]) : J * polaron_propagator(t, v[i], w[i], β); dims = dims) * ω
    end
    return Holstein(ω .* ω0_pu, J .* E0_pu, dims, g .* E0_pu, α, E .* E0_pu, v .* ω0_pu, w .* ω0_pu, β / E0_pu, 0 * ω0_pu, Σ .* ω0_pu)
end
...
function holstein(α::Number, ω::Number, J::Number, β; dims = 3, verbose = false)
    ...
    @inbounds for i in 1:num_β
        ...
        S(v, w) = holstein_S(v, w, g, τ -> β[i] == Inf ? phonon_propagator(τ, ω) : phonon_propagator(τ, ω, β[i]), (τ, v, w) -> β[i] == Inf ? J * polaron_propagator(τ, v, w) : J * polaron_propagator(τ, v, w, β[i]), limits = [0, β[i]/2]; dims = dims)
        ...
        @fastmath Σ[i] = holstein_memory(eps(), g, t -> β[i] == Inf ? phonon_propagator(t, ω) : phonon_propagator(t, ω, β[i]), t -> β[i] == Inf ? J * polaron_propagator(t, v[i], w[i]) : J * polaron_propagator(t, v[i], w[i], β[i]); dims = dims) * ω
...
function holstein(α, ω::Number, J::Number, β; dims = 3, verbose = false)
    ...
    @inbounds for j in 1:num_β, i in 1:num_α
        ...
        @fastmath g[i] = pustrip(holstein_coupling(α[i], ω * ω0_pu, J * E0_pu, dims))
        S(v, w) = holstein_S(v, w, g[i], τ -> β[j] == Inf ? phonon_propagator(τ, ω) : phonon_propagator(τ, ω, β[j]), (τ, v, w) -> β[j] == Inf ? J * polaron_propagator(τ, v, w) : J * polaron_propagator(τ, v, w, β[j]), limits = [0, β[j]/2]; dims = dims)
        ...
        @fastmath Σ[i, j] = holstein_memory(eps(), g[i], t -> β[j] == Inf ? phonon_propagator(t, ω) : phonon_propagator(t, ω, β[j]), t -> β[j] == Inf ? J * polaron_propagator(t, v[i, j], w[i, j]) : J * polaron_propagator(t, v[i, j], w[i, j], β[j]); dims = dims) * ω
        if verbose n += 1; print("\e[1F") end
    end
    return Holstein(ω .* ω0_pu, J .* E0_pu, dims, g .* E0_pu, α, E .* E0_pu, v .* ω0_pu, w .* ω0_pu, β ./ E0_pu, 0 * ω0_pu, Σ .* ω0_pu)
end
...
function holstein(α::Number, ω::Number, J::Number, β::Number, Ω::Number; dims = 3, verbose = false)
    ...
    Σ = holstein_memory(Ω, g, t -> β == Inf ? phonon_propagator(t, ω) : phonon_propagator(t, ω, β), t -> β == Inf ? J * polaron_propagator(t, v, w) : J * polaron_propagator(t, v, w, β); dims = dims) * ω
...
function holstein(α, ω::Number, J::Number, β::Number, Ω::Number; dims = 3, verbose = false)
    ...
    @inbounds for i in 1:num_α
        ...
        S(v, w) = holstein_S(v, w, g[i], τ -> β == Inf ? phonon_propagator(τ, ω) : phonon_propagator(τ, ω, β), (τ, v, w) -> β == Inf ? J * polaron_propagator(τ, v, w) : J * polaron_propagator(τ, v, w, β), limits = [0, β/2]; dims = dims)
        @fastmath v[i], w[i], E[i] = variation((v, w) -> β == Inf ? -2 * dims * J + E₀(v, w) * dims / 3 - (S(v, w) - S₀(v, w) * dims / 3) : -2 * dims * J + E₀(v, w, β) * dims / 3 - (S(v, w) - S₀(v, w, β) * dims / 3), i > 1 ? v[i-1] : 2 * dims + 1 / β, i > 1 ? w[i-1] : ω + 1 / β)
        @fastmath Σ[i] = holstein_memory(Ω, g[i], t -> β == Inf ? phonon_propagator(t, ω) : phonon_propagator(t, ω, β), t -> β == Inf ? J * polaron_propagator(t, v[i], w[i]) : J * polaron_propagator(t, v[i], w[i], β); dims = dims) * ω
        if verbose n += 1; print("\e[1F") end
    end
    return Holstein(ω .* ω0_pu, J .* E0_pu, dims, g .* E0_pu, α, E .* E0_pu, v .* ω0_pu, w .* ω0_pu, β ./ E0_pu, Ω .* ω0_pu, Σ .* ω0_pu)
end
...
function holstein(α::Number, ω::Number, J::Number, β, Ω::Number; dims = 3, verbose = false)
    ...
    @inbounds for j in 1:num_β
        ...
        S(v, w) = holstein_S(v, w, g, τ -> β[j] == Inf ? phonon_propagator(τ, ω) : phonon_propagator(τ, ω, β[j]), (τ, v, w) -> β[j] == Inf ? J * polaron_propagator(τ, v, w) : J * polaron_propagator(τ, v, w, β[j]), limits = [0, β[j]/2]; dims = dims)
        ...
        @fastmath Σ[j] = holstein_memory(Ω, g, t -> β[j] == Inf ? phonon_propagator(t, ω) : phonon_propagator(t, ω, β[j]), t -> β[j] == Inf ? J * polaron_propagator(t, v[j], w[j]) : J * polaron_propagator(t, v[j], w[j], β[j]); dims = dims) * ω
        if verbose n += 1; print("\e[1F") end
    end
    return Holstein(ω .* ω0_pu, J .* E0_pu, dims, g .* E0_pu, α, E .* E0_pu, v .* ω0_pu, w .* ω0_pu, β ./ E0_pu, Ω .* ω0_pu, Σ .* ω0_pu)
end
...
function holstein(α::Number, ω::Number, J::Number, β::Number, Ω; dims = 3, verbose = false)
    ...
    @inbounds for k in 1:num_Ω
        ...
        @fastmath Σ[k] = holstein_memory(Ω[k], g, t -> β == Inf ? phonon_propagator(t, ω) : phonon_propagator(t, ω, β), t -> β == Inf ? J * polaron_propagator(t, v, w) : J * polaron_propagator(t, v, w, β); dims = dims) * ω
        if verbose n += 1; print("\e[1F") end
    end
    return Holstein(ω .* ω0_pu, J .* E0_pu, dims, g .* E0_pu, α, E .* E0_pu, v .* ω0_pu, w .* ω0_pu, β ./ E0_pu, Ω .* ω0_pu, Σ .* ω0_pu)
end
...
function holstein(α, ω::Number, J::Number, β::Number, Ω; dims = 3, verbose = false)
    ...
    @inbounds for i in 1:num_α
        ...
        @inbounds for k in 1:num_Ω
            ...
            @fastmath Σ[i, k] = holstein_memory(Ω[k], g[i], t -> β == Inf ? phonon_propagator(t, ω) : phonon_propagator(t, ω, β), t -> β == Inf ? J * polaron_propagator(t, v[i], w[i]) : J * polaron_propagator(t, v[i], w[i], β); dims = dims) * ω
            if verbose n += 1; print("\e[1F") end
        end
...
function holstein(α::Number, ω::Number, J::Number, β, Ω; dims = 3, verbose = false)
    ...
    @inbounds for j in 1:num_β
        ...
        @inbounds for k in 1:num_Ω
            ...
            @fastmath Σ[j, k] = holstein_memory(Ω[k], g, t -> β[j] == Inf ? phonon_propagator(t, ω) : phonon_propagator(t, ω, β[j]), t -> β[j] == Inf ? J * polaron_propagator(t, v[j], w[j]) : J * polaron_propagator(t, v[j], w[j], β[j]); dims = dims) * ω
            if verbose n += 1; print("\e[1F") end
        end
    end
    return Holstein(ω .* ω0_pu, J .* E0_pu, dims, g .* E0_pu, α, E .* E0_pu, v .* ω0_pu, w .* ω0_pu, β ./ E0_pu, Ω .* ω0_pu, Σ .* ω0_pu)
...
function holstein(α, ω::Number, J::Number, β, Ω::Number; dims = 3, verbose = false)
    ...
    @inbounds for j in 1:num_β, i in 1:num_α
        ...
        @fastmath Σ[i, j] = holstein_memory(Ω, g[i], t -> β[j] == Inf ? phonon_propagator(t, ω) : phonon_propagator(t, ω, β[j]), t -> β[j] == Inf ? J * polaron_propagator(t, v[i, j], w[i, j]) : J * polaron_propagator(t, v[i, j], w[i, j], β[j]); dims = dims) * ω
        if verbose n += 1; print("\e[1F") end
    end
    return Holstein(ω .* ω0_pu, J .* E0_pu, dims, g .* E0_pu, α, E .* E0_pu, v .* ω0_pu, w .* ω0_pu, β ./ E0_pu, Ω .* ω0_pu, Σ .* ω0_pu)
end
...
function holstein(α, ω::Number, J::Number, β, Ω; dims = 3, verbose = false)
    ...
    @inbounds for j in 1:num_β, i in 1:num_α
        ...
        @inbounds for k in 1:num_Ω
            ...
            @fastmath Σ[i, j, k] = holstein_memory(Ω[k], g[i], t -> β[j] == Inf ? phonon_propagator(t, ω) : phonon_propagator(t, ω, β[j]), t -> β[j] == Inf ? J * polaron_propagator(t, v[i, j], w[i, j]) : J * polaron_propagator(t, v[i, j], w[i, j], β[j]); dims = dims) * ω
            if verbose n += 1; print("\e[1F") end
        end
    end
    return Holstein(ω .* ω0_pu, J .* E0_pu, dims, g .* E0_pu, α, E .* E0_pu, v .* ω0_pu, w .* ω0_pu, β ./ E0_pu, Ω .* ω0_pu, Σ .* ω0_pu)
...
function holstein_S(v, w, coupling, phonon_propagator, polaron_propagator; limits = [0, Inf], dims = 3)
    integral, _ = quadgk(τ -> phonon_propagator(τ) * (sqrt(π / J * polaron_propagator(τ, v, w)) * erf(π * sqrt(J * polaron_propagator(τ, v, w))))^dims, limits...)
    return norm(coupling)^2 * integral / (2π)^dims / 2
end

function holstein_memory(Ω, coupling, phonon_propagator, polaron_propagator; dims = 3)
    integral, _ = quadgk(t -> (1 - exp(im * Ω * t)) / Ω * imag(phonon_propagator(im * t) * (√π / 2 * erf(π * sqrt(J * polaron_propagator(im * t))) / J * polaron_propagator(im * t)^(3/2) - π * exp(-π^2 * J * polaron_propagator(im * t)) / J * polaron_propagator(im * t)) * (√π * erf(π * sqrt(J * polaron_propagator(im * t))) / sqrt(J * polaron_propagator(im * t)))^(dims - 1)), 0, Inf)
