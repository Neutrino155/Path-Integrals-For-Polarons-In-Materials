...
function multiholstein(α::AbstractArray, ω::AbstractArray, J::Number, β::Number; dims = 3, v_guesses = false, w_guesses = false, upper = Inf, verbose = false)
    ω = pustrip.(ω)
    J = pustrip(J)
    β = pustrip(β * ħ_pu / E0_pu) 
    g = pustrip.(holstein_coupling.(α, ω * ω0_pu, J * E0_pu, dims))
...
function holstein(α, ω, J, β; verbose = false, dims = 3, v_guesses = false, w_guesses = false, upper = Inf, kwargs...)
    ...
    Threads.@threads :static for x in CartesianIndices((num_α, num_ω, num_J, num_β))
        ...
        @views holsteins[x] = holstein(α[x[1]], ω[x[2]], J[x[3]], β[x[4]]; v_guesses = v_guess[id], w_guesses = w_guess[id], dims = dims, kwargs...)
        v_guess[id], w_guess[id] = pustrip.(holsteins[x].v), pustrip.(holsteins[x].w)
    end
...
function holstein(h::Holstein, Ω; dims = 3, verbose = false, kwargs...)
    ...
    Threads.@threads for x in CartesianIndices((num_α, num_ω, num_J, num_β, num_Ω))
        ...
        @views Σ[x] = holstein_memory(Ω[x[5]], g[x[1],x[2],x[3]], t -> β[x[4]] == Inf ? phonon_propagator(t, ω[x[2]]) : phonon_propagator(t, ω[x[2]], β[x[4]]), t -> β[x[4]] == Inf ? polaron_propagator(t, v[x[1],x[2],x[3],x[4]], w[x[1],x[2],x[3],x[4]]) * J[x[3]] : polaron_propagator(t, v[x[1],x[2],x[3],x[4]], w[x[1],x[2],x[3],x[4]], β[x[4]]) * J[x[3]]; dims = dims) * J[x[3]]
        h.Ω, h.Σ = Ω .* ω0_pu, reduce_array(Σ) .* ω0_pu
    end
    return holstein(h)
end

function multiholstein(α, ω, J, β; verbose = false, dims = 3, v_guesses = false, w_guesses = false, upper = Inf, kwargs...)
    num_J, num_β = length(J), length(β)
    if verbose N, n = num_J * num_β, Threads.Atomic{Int}(1) end
    hstart = holstein(; dims = dims, v_guesses = v_guesses, w_guesses = w_guesses, kwargs...)
    v_guess, w_guess = fill(pustrip.(hstart.v), Threads.nthreads()), fill(pustrip.(hstart.w), Threads.nthreads())
    holsteins = fill(hstart, num_J, num_β)
    Threads.@threads :static for x in CartesianIndices((num_J, num_β))
        ...
        @views holsteins[x] = holstein(α, ω, J[x[1]], β[x[2]]; v_guesses = v_guess[id], w_guesses = w_guess[id], dims = dims, upper = upper, kwargs...)
        v_guess[id], w_guess[id] = pustrip.(holsteins[x].v), pustrip.(holsteins[x].w)
    end
    polaron = holstein(holsteins)
    polaron.α, polaron.g, polaron.d, polaron.ω, polaron.J, polaron.β, polaron.Ω, polaron.Σ = α, reduce_array(polaron.g[:,1]), dims, pustrip.(ω) * ω0_pu, pustrip.(J) .* E0_pu, pustrip.(β) / E0_pu, zero(Float64) * ω0_pu, zero(Complex) * ω0_pu
